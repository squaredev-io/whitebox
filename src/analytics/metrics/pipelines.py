...
def create_data_drift_pipeline(reference_dataset: pd.DataFrame,current_dataset: pd.DataFrame)-> Dict[str,Any]:
    """
    You will need two datasets. The reference dataset serves as a benchmark. We analyze the change by comparing the current 
    production data to the reference data.
    ...
    You can potentially choose any two datasets for comparison. But keep in mind that only the reference dataset 
